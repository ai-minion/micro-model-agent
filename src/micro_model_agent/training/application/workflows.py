...
class RunSyntheticTrainingWorkflow:
    ...
    1. Validate and export the dataset.
    2. Build a ``TrainingJob`` aggregate and call ``start()`` / ``complete()`` / ``fail()``.
    3. Persist artifacts via ``artifact_store``.
    4. Publish domain events (including ``ArtifactProduced``) via the optional event bus.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._config_validator = TrainingConfigValidationService()
    ...
    async def run(
        ...
        # Use the TrainingJob aggregate so lifecycle events are captured.
        # Validate config first — domain service raises InvalidConfigError on violations.
        self._config_validator.validate(config)
        job = TrainingJob(config=config)
        run = await self.runner.run(config)

        if run.status.value == "succeeded" and run.artifacts:
            first_artifact = run.artifacts[0]
            job.start(run)
            job.complete(run, first_artifact)
            for artifact in run.artifacts:
                await self.artifact_store.save(artifact)
        else:
            job.start(run)
            job.fail(run.error or "runner reported failure")

        if self.event_bus is not None:
            await self.event_bus.publish_all(job.pull_events())
