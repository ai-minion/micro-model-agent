"""Build the decision-labeled workspace-process training mix for run 0020."""
...
OUTPUT_PATH = Path(
    ".micro_model_agent/datasets/decision_labeled_workspace_process_0020_training.jsonl"
)
...
def main() -> None:
    ...
    trace_protectors = _clone_category_counts(
        old_records,
        TRACE_PROTECTOR_COUNTS,
        run_label="0020-trace-protector",
    )
    positive_schema = _clone_category_counts(
        old_records,
        POSITIVE_SCHEMA_COUNTS,
        run_label="0020-positive-schema",
    )
    repair_refusal = _clone_category_counts(
        old_records,
        REPAIR_REFUSAL_COUNTS,
        run_label="0020-small-repair-refusal",
    )
    ...
    _require_count("0020 training mix", records, 600)
...
def _clone_for_run(
    ...
    *,
    ...
    metadata["reinforcement_run"] = "0020"
